...
from .serlalizers import RecoverPasswordSerializer, ResetPasswordSerializer, UserSerializer, RegisterSerializer, LoginSerializer, DeleteAccountSerializer, ChangePasswordSerializer
...
class RegisterWithSendingEmailView(APIView):
    ...
    def __send_account_activation_email(self, user):
        ...
        # activation_link = f"http://localhost:5173/activate/{uid}/{token}/" # if we use React with Vite in development mode.
        activation_link = f"http://localhost:4173/activate/{uid}/{token}/" # if we use React with Vite in production preview mode.
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                user_with_email = User.objects.get(email=data["email"])

                if user_with_email:
                    self.__send_account_activation_email(user_with_email)
                    return Response(f"Dear user {username}. Please check your email {email} to activate your account.", status=status.HTTP_201_CREATED)
        
            except User.DoesNotExist:
                user_with_email=None


            # The created user is inactive until email is verified.
            user = User.objects.create_user(email=email, username=serializer.data["username"], password=password)

            self.__send_account_activation_email(user)

            return Response(f"Please check your email {email} to activate your account.", status=status.HTTP_201_CREATED)
...
class ActivateAccountView(APIView):
    def post(self, request, uidb64, token):
        ...
        except (TypeError, ValueError, OverflowError, BadSignature):
            user = None
...
class RequestChangeForgottenPasswordView(APIView):
    ...
    def __send_recover_password_email(self, user):
        ...
        # activation_link = f"http://localhost:5173/confirmChangeForgottenPassword/{uid}/{token}/" # if we use React with Vite in development mode.
        activation_link = f"http://localhost:4173/confirmChangeForgottenPassword/{uid}/{token}/" # if we use React with Vite in production preview mode.
    ...
    def post(self, request):
        # extract email from the data passed with the request (the request body)
        data = {
            "email": request.data["email"],
        }
        ...
        if serializer.is_valid():
            ...
            return Response(f"Please check your email {email} to recover your password.", status=status.HTTP_200_OK)
